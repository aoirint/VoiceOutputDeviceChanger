...
The event-owned Pull Request and Main workflows are enabled. GitHub prerelease
publication is enabled for SemVer prerelease versions. Stable GitHub and
Thunderstore publication remain disabled. APM manages repository-local Agent
Skills and is not a mod-package host.
...
| Release classification | Confirmed | `0.0.0` is edge; SemVer prereleases publish to GitHub; a nonzero three-part numeric version is stable |
| Publication authorization | Partial | GitHub prereleases are enabled; stable GitHub and Thunderstore publication are disabled |
...
The current runtime gaps do not block a clearly labeled GitHub alpha.
They must be closed before stable GitHub or Thunderstore publication.
...
Before enabling stable publication, use a clean Windows BepInEx 5 profile with
no other mods and confirm:
...
3. Set `Version` in `VoiceOutputDeviceChanger/VoiceOutputDeviceChanger.csproj`
   to the same prerelease version.
4. Record any pending runtime validation in both changelogs.
5. Run every development, workflow, package-contract, and documentation check.
6. Push the reviewed commit to `main`.

CI keeps the prerelease identity in the assembly metadata, ZIP name, tag, and
...
6. Explicitly enable stable GitHub and Thunderstore publication in the
   committed workflow.
